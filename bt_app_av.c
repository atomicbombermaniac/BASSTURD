...
static uint8_t s_volume = 10;                 /* local volume value */
...
void volume_set_by_local_host(uint8_t volume)
{
    ESP_LOGI(BT_RC_TG_TAG, "Volume is set locally to: %"PRIu32"%%", (uint32_t)volume);
    /* set the volume in protection of lock */
    _lock_acquire(&s_volume_lock);
    s_volume = volume;
    _lock_release(&s_volume_lock);
...
{
    uint8_t data_scaled[4096];
    int16_t *input = (int16_t *)data;
    int16_t *output=  (int16_t *)data_scaled;
    uint16_t i;

    volatile uint8_t temp_level_l = 0, temp_level_r = 0;
    temp_level_l = 0;
    temp_level_r = 0;

    for(i = 0; i< len/4; i++)
    {
        temp_level_l = _max((_abs(*input))/355, temp_level_l);

        if(s_volume)
            *output = (int16_t)((*input)* 0.2 *  (float)((float)((float)s_volume * (float)s_volume/100.0)/100.0));
        else
            *output = 0;
        input++;
        output++;

        temp_level_r = _max((_abs(*input))/355, temp_level_r);
    ...
    level_l = temp_level_l;
    level_r = temp_level_r;

    write_ringbuf(data_scaled, len);

    /* log the number every 100 packets */
    //if (++s_pkt_cnt % 100 == 0) {
    //    ESP_LOGI(BT_AV_TAG, "Audio packet len: %"PRIu32, len);
        //uint16_t *p = data;
        //uint16_t *pp = &data_scaled;
        //int32_t p1 = (int32_t)*p;
        //int32_t p2 = (int32_t)*pp;
        //uint16_t intermed =  *(input-10);
        //int32_t p2 = intermed;
        //ESP_LOGI(BT_AV_TAG, "Data: %"PRIu32, p1);
        //ESP_LOGI(BT_AV_TAG, "Data_out: %"PRIu32, p2);
        //ESP_LOGI(BT_AV_TAG, "Len: %"PRIu32,(uint32_t) temp_input);
        //ESP_LOGI(BT_AV_TAG, "Len: %"PRIu32,(uint32_t) level_l);
    //}
}
